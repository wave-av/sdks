...
impl<'a> Chapters<'a> {
    ...
    pub fn list(&self, video_id: &str) -> Result<serde_json::Value, Error> {
        let path = "/videos/{videoId}/chapters"
            .to_string()
    ...
    pub fn create(
        &self,
        video_id: &str,
        body: &models::ChapterCreate,
    ) -> Result<models::Chapter, Error> {
        let path = "/videos/{videoId}/chapters"
            .to_string()
            .replace("{videoId}", video_id);
    ...
    pub fn detect(
        &self,
        video_id: &str,
        body: &models::ChapterDetectRequest,
    ) -> Result<models::DetectionJob, Error> {
        let path = "/videos/{videoId}/chapters/detect"
            .to_string()
            .replace("{videoId}", video_id);
        self.http.request("POST", &path, &[], Some(body))
    }
