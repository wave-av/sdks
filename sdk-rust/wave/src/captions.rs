...
impl<'a> Captions<'a> {
    ...
    pub fn list(&self, params: &CaptionsListParams) -> Result<Page<models::CaptionJob>, Error> {
        ...
        self.http.request("GET", "/captions", &query, None::<&serde_json::Value>)
    }
    ...
    pub fn download(&self, job_id: &str, language: &str, params: &CaptionsDownloadParams) -> Result<serde_json::Value, Error> {
        let path = "/captions/{jobId}/download".to_string().replace("{jobId}", job_id);
        let mut query: Vec<(String, String)> = Vec::new();
