...
impl<'a> Transcribe<'a> {
    ...
    pub fn list_transcriptions(
        &self,
        params: &TranscribeListTranscriptionsParams,
    ) -> Result<Page<models::Transcription>, Error> {
        ...
        self.http
            .request("GET", "/transcribe", &query, None::<&serde_json::Value>)
    }

    /// Create a transcription (operationId: createTranscription, POST /transcribe).
    pub fn create_transcription(
        &self,
        body: &models::TranscriptionCreate,
    ) -> Result<models::Transcription, Error> {
    ...
    pub fn get_transcription(
        ...
        let path = "/transcribe/{transcriptionId}"
            .to_string()
            .replace("{transcriptionId}", transcription_id);
        self.http
