...
Typed client for **WAVE Ghost** through the WAVE gateway. Entitlement is enforced server-side (401 unauthenticated / 403 under-scoped) — installing this package does not grant access.

```bash
npm i @wave-av/ghost      # pulls @wave-av/core automatically
```

```ts
import { createClient } from "@wave-av/core";
import { createGhostAPI } from "@wave-av/ghost";

const api = createGhostAPI(createClient({ apiKey: process.env.WAVE_API_KEY! }));
```
