...
Typed client for **WAVE Billing** through the WAVE gateway. Entitlement is enforced server-side (401 unauthenticated / 403 under-scoped) — installing this package does not grant access.

```bash
npm i @wave-av/billing      # pulls @wave-av/core automatically
```

```ts
import { createClient } from "@wave-av/core";
import { createBillingAPI } from "@wave-av/billing";

const api = createBillingAPI(createClient({ apiKey: process.env.WAVE_API_KEY! }));
```
