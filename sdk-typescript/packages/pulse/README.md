...
Typed client for **WAVE Pulse** through the WAVE gateway. Entitlement is enforced server-side (401 unauthenticated / 403 under-scoped) — installing this package does not grant access.

```bash
npm i @wave-av/pulse      # pulls @wave-av/core automatically
```

```ts
import { createClient } from "@wave-av/core";
import { createPulseAPI } from "@wave-av/pulse";

const api = createPulseAPI(createClient({ apiKey: process.env.WAVE_API_KEY! }));
```
