...
Typed client for **WAVE QR** through the WAVE gateway. Entitlement is enforced server-side (401 unauthenticated / 403 under-scoped) — installing this package does not grant access.

```bash
npm i @wave-av/qr      # pulls @wave-av/core automatically
```

```ts
import { createClient } from "@wave-av/core";
import { createQrAPI } from "@wave-av/qr";

const api = createQrAPI(createClient({ apiKey: process.env.WAVE_API_KEY! }));
```
