...
Typed client for **WAVE Desktop** through the WAVE gateway. Entitlement is enforced server-side (401 unauthenticated / 403 under-scoped) — installing this package does not grant access.

```bash
npm i @wave-av/desktop      # pulls @wave-av/core automatically
```

```ts
import { createClient } from "@wave-av/core";
import { createDesktopAPI } from "@wave-av/desktop";

const api = createDesktopAPI(createClient({ apiKey: process.env.WAVE_API_KEY! }));
```
