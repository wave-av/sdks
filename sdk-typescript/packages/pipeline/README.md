...
Typed client for **WAVE Pipeline** through the WAVE gateway. Entitlement is enforced server-side (401 unauthenticated / 403 under-scoped) — installing this package does not grant access.

```bash
npm i @wave-av/pipeline      # pulls @wave-av/core automatically
```

```ts
import { createClient } from "@wave-av/core";
import { createPipelineAPI } from "@wave-av/pipeline";

const api = createPipelineAPI(createClient({ apiKey: process.env.WAVE_API_KEY! }));
```
