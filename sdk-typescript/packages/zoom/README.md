...
Typed client for **WAVE Zoom** through the WAVE gateway. Entitlement is enforced server-side (401 unauthenticated / 403 under-scoped) — installing this package does not grant access.

```bash
npm i @wave-av/zoom      # pulls @wave-av/core automatically
```

```ts
import { createClient } from "@wave-av/core";
import { createZoomAPI } from "@wave-av/zoom";

const api = createZoomAPI(createClient({ apiKey: process.env.WAVE_API_KEY! }));
```
