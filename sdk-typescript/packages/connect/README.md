...
Typed client for **WAVE Connect** through the WAVE gateway. Entitlement is enforced server-side (401 unauthenticated / 403 under-scoped) — installing this package does not grant access.

```bash
npm i @wave-av/connect      # pulls @wave-av/core automatically
```

```ts
import { createClient } from "@wave-av/core";
import { createConnectAPI } from "@wave-av/connect";

const api = createConnectAPI(createClient({ apiKey: process.env.WAVE_API_KEY! }));
```
